...
class CheckpointSaver:
    def __init__(
        ...
        self.checkpoint_dir_bak = checkpoint_dir_bak
    ...
    def save_checkpoint(self, model, optimizer, args, epoch, model_ema=None, metric=None, use_amp=False):
        ...
        if (len(self.checkpoint_files) < self.max_history
            ...
            if metric is not None and (self.best_metric is None or self.cmp(metric, self.best_metric)):
                ...
                # print('Metric:',metric)
                # if self.checkpoint_dir_bak != '' and metric > 90:
                if self.checkpoint_dir_bak != '':
                    shutil.copyfile(save_path, os.path.join(self.checkpoint_dir_bak, 'model_best' + self.extension))
...
def accuracy(output, target, topk=(1,)):
    ...
    if len(topk) == 1:
        return correct[:topk[0]].view(-1).float().sum(0) * 100. / batch_size
    else:
...
def plot_figure(x_data, y_data, name, plots_dir):
    output_file = os.path.join(plots_dir, '{}.jpg'.format(name))
    ncols = 1
    nrows = 1
    fig, axs = plt.subplots(ncols=ncols, nrows=nrows, figsize=(ncols * 6, nrows * 6))
    # ax = axs[0]
    ax = axs
...
def plot_csv(filename, plots_dir):
    if not os.path.isdir(plots_dir):
        os.mkdir(plots_dir)
    data = pd.read_csv(filename)
    columns = data.columns.tolist()
    x_data = list(data['epoch'])

    for column in columns:
        if column == 'epoch':
            continue
        y_data = list(data[column])
        plot_figure(x_data, y_data, column, plots_dir)


def update_summary(epoch, train_metrics, eval_metrics, filename, plots_dir, write_header=False):
