...
default_cfgs = {
    ...
    'efficientnet_deepfake': _cfg(
        url='', input_size=(12, 600, 600), pool_size=(19, 19), crop_pct=0.949),
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, num_features=1280, in_chans=3, stem_size=32,
        ...
        # print(norm_kwargs,stem_size,channel_multiplier,channel_divisor,channel_min)
        self.num_classes = num_classes
        self.num_features = num_features
        self.drop_rate = drop_rate
        self._in_chs = in_chans

        # norm_kwargs = {'affine':False, 'track_running_stats':False}
        # Stem
        stem_size = round_channels(stem_size, channel_multiplier, channel_divisor, channel_min)
        self.conv_stem = create_conv2d(self._in_chs, stem_size, 3, stride=2, padding=pad_type)
        self.bn1 = norm_layer(stem_size, **norm_kwargs)
        # self.bn1 = norm_layer(stem_size, track_running_stats=False)
        # self.bn1 = nn.BatchNorm2d(stem_size, track_running_stats=False)
        ...
        # self.bn2 = norm_layer(self.num_features, momentum=0)
        # self.bn2 = nn.BatchNorm2d(self.num_features, track_running_stats=False)
    ...
    def forward_features(self, x):
        ...
        # print(x.shape)
        x = self.blocks(x)
        # print('blocks out shape', x.shape)
        # print(x.shape)
        x = self.conv_head(x)
        # print('conv head shape', x.shape)
        x = self.bn2(x)
        x = self.act2(x)
        return x
    ...
    def forward(self, x):
        ...
        # print('foward shape', x.shape)
        # print(x.shape)  #[1, 2560, 19, 19]
        x = self.global_pool(x)
        # print('pool shape', x.shape)
        # print(x.shape) # [1, 2560, 1, 1]
        x = x.flatten(1)
        # print('flatten shape', x.shape)
        # print(x.shape) # [1, 2560]
        if self.drop_rate > 0.:
...
def _gen_efficientnet_deepfake(variant, channel_multiplier=1.0, depth_multiplier=1.0, pretrained=False, **kwargs):
    """Creates an EfficientNet model.
    ...
    # change feature size and stem size
    model_kwargs = dict(
        block_args=decode_arch_def(arch_def, depth_multiplier),
        num_features=round_channels(128, channel_multiplier, 8, None),
        stem_size=128,
        channel_multiplier=channel_multiplier,
        act_layer=Swish,
        norm_kwargs=resolve_bn_args(kwargs),
...
@register_model
def efficientnet_deepfake(pretrained=False, **kwargs):
    """ EfficientNet-B7 """
    # NOTE for train, drop_rate should be 0.5, drop_path_rate should be 0.2
    model = _gen_efficientnet_deepfake(
        'efficientnet_deepfake', channel_multiplier=2.0, depth_multiplier=3.1, pretrained=pretrained, **kwargs)
    return model
